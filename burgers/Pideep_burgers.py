...
from models.models_baselines import PI_Deeponet
...
def main(args):
    ...
    if args.data_use:
        name = 'deeponet-physics'
    else:
        name = 'deeponet-hybrid'
    ...
    criterion = torch.nn.SmoothL1Loss(beta=0.5)
    # create model
    para_layer =  [101, 64, 64, 64, 64, 64]
    truck_layer = [2, 64, 64, 64, 64, 64]
    model = PI_Deeponet(para_layer, truck_layer)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch% 10==0 and epoch >0:
            if args.data_use:
                torch.save(model, rf'{args.log_dir}/deeponet_burgers_hybrid.pkl')
            else:
                torch.save(model, rf'{args.log_dir}/deeponet_burgers_physics.pkl')
