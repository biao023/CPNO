    ...
    for epoch in range(args.epochs):
        ...
        if epoch > 0:
            with torch.no_grad():
                ...
                #torch.save(model, rf'logs/polyINR_burgers_physics.pkl')
                if args.data_use:
                    logging.info(
        ...
        if epoch> 0 and epoch%10==0:
            if args.data_use:
                torch.save(model, rf'{args.log_dir}/polyINR_cheb_burgers_hybrid.pkl')
            else:
                torch.save(model, rf'{args.log_dir}/polyINR_cheb_burgers_physics.pkl')
