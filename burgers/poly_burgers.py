...
def main(args):
    args.log_dir = './logs_burgers_sum'
    if args.data_use:
        name = 'PolyINR-hybrid={}-epoch={}-lr={}-inr_dim_hidden={}-' \
           'inr_layers={}-hyper_dim_hidden={}-hyper_num_layers={}-c_IC={}-c_BC={}-c_PDE={}'.format(
        args.dataset,  args.epochs, args.lr, args.inr_dim_hidden,
        args.inr_num_layers, args.hyper_dim_hidden,args.hyper_num_layers, args.c_ic, args.c_bc, args.c_pde)
    else:
    ...
    for epoch in range(args.epochs):
        ...
        if epoch% 10==0 and epoch >0:
            if args.data_use:
                torch.save(model, rf'{args.log_dir}/polyINR_burgers_hybrid.pkl')
            else:
                torch.save(model, rf'{args.log_dir}/polyINR_burgers_physics.pkl')
