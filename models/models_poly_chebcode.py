...
class PolyINR(nn.Module):
    ...
    def __init__(self,
        ...
        self.affines = nn.ModuleList([UniformInitDense(num_bases*2 +2 , dim_hidden) for _ in range(num_layers - 1)])
    ...
    def forward(self,
        ...
        for i in range(self.num_bases + 1):
            if i == 0:
                T = torch.ones_like(x)  # (B, N, D)
            elif i == 1:
                T = x
            else:
                T = 2 * x * features[i-1] - features[i-2]
            features.append(T)
        x_pad = torch.cat(features[1:], dim=-1)
        x_pad = torch.cat((x_pad, x), dim=-1)
        ...
        for layer_idx in range(self.num_layers - 1):
            ...
            tmp = self.affines[layer_idx](x_pad)  # [bsz, n_pts, dim_hidden]
...
class PolyINRWithHypernet(nn.Module):
    ...
    def forward(self, hyper_in: torch.Tensor, coordinate: torch.Tensor,) -> torch.Tensor:
        if self.enable_affine:
            affine_modulations = []
